...
SELECT
  p."id",
  LOWER(REGEXP_REPLACE(COALESCE(p."name", p."id"), '[^a-zA-Z0-9]+', '-', 'g')) || '-' || LEFT(p."id", 8),
  COALESCE(p."name", '未命名作品'),
  COALESCE(p."description", ''),
  COALESCE(p."category", 'uncategorized'),
  COALESCE(p."priceType"::TEXT, 'inquiry')::"WorkPriceType",
  p."price",
  p."priceMin",
  p."priceMax",
  COALESCE(p."currency", 'TWD'),
  COALESCE(p."isAvailable", true),
  COALESCE(p."imageUrl", ''),
...
FROM "products" p
WHERE NOT EXISTS (
  SELECT 1 FROM "works" w WHERE w."id" = p."id"
);
