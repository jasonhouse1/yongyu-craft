  ...
  return (
    <>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 500, color: "#e5e5e5", marginBottom: 4 }}>新增活動橫幅</h1>
        ...
        <div style={{ background: "#111", border: "1px solid #1f1f1f", borderRadius: 8, padding: "28px 24px" }}>
          <form onSubmit={handleSubmit}>
            <div style={group}>
              ...
              <input style={field} value={form.title} onChange={e => set("title", e.target.value)} placeholder="春季特展開幕" />
            </div>

            <div style={group}>
              <label style={label}>副標題</label>
              <input style={field} value={form.subtitle} onChange={e => set("subtitle", e.target.value)} placeholder="限時優惠 · 手工珠寶展" />
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
              <div>
                <label style={label}>按鈕文字</label>
                <input style={field} value={form.ctaText} onChange={e => set("ctaText", e.target.value)} placeholder="了解更多" />
              </div>
              <div>
                <label style={label}>連結網址</label>
                <input style={field} value={form.ctaUrl} onChange={e => set("ctaUrl", e.target.value)} placeholder="/works" />
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
              <div>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <input style={{ ...field, flex: 1 }} value={form.bgColor} onChange={e => set("bgColor", e.target.value)} placeholder="#0D0C0B" />
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input type="color" value={form.accentColor} onChange={e => set("accentColor", e.target.value)}
                    style={{ width: 36, height: 32, padding: 2, background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 4, cursor: "pointer" }} />
                  <input style={{ ...field, flex: 1 }} value={form.accentColor} onChange={e => set("accentColor", e.target.value)} placeholder="#C49A5A" />
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                ...
                <span style={{ color: "#888", fontSize: 13 }}>立即啟用</span>
              </label>
            </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 48px 0 16px" }}>
                ...
                {form.subtitle && (
                  <>
                    ...
                    <span style={{ fontSize: "0.8rem", color: form.accentColor, letterSpacing: "0.08em", fontWeight: 300, whiteSpace: "nowrap" }}>
                      {form.subtitle}
                    </span>
                  </>
                )}
                {form.ctaText && (
                  <>
                    <span style={{ color: form.accentColor, opacity: 0.5 }}>·</span>
                    <span style={{ fontSize: "0.75rem", color: form.accentColor, letterSpacing: "0.12em", borderBottom: `1px solid ${form.accentColor}`, whiteSpace: "nowrap" }}>
                      {form.ctaText}
                    </span>
                  </>
                )}
